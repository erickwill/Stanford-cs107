...
static void ParseArticle(const char *articleTitle,const char *articleDescription,
			 const char *articleURL,rssDatabase *db,
			 bool needToFreeConnection);
void ParseArticleHelper(article *currArt, urlconnection *urlconn,
			url *u, rssDatabase *db);
static void *ScanArticle(void *scanPackage);
static void ProcessWellFormedWord(char *word, article *a, rssDatabase *db);
static void UpdateOccurences(vector *articles, article *a);
static void QueryIndices(rssDatabase *db);
static void ProcessResponse(const char *word, rssDatabase *db);
static void ProcessValidResponse(const char *word, rssDatabase *db);
...
int main(int argc, char **argv)
{
  ...
  BuildStopWordsHashset(&db.stopWords, kDefaultStopWordsFile);
  
  HashSetNew(&db.wordHash, sizeof(currWord), 10007, WordHashFn, 
	     WordCompare, WordFree);

  HashSetNew(&db.articlesSeen, sizeof(article), 10007, ArticleHashFn, 
	     ArticleCompare, ArticleFree);

  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &db);
...
static void SetupSemaphores(rssDatabase *db)
{
  sem_init(&db->totalNetworkConnections,0,24);
...
static void BuildIndices(const char *feedsFileName, rssDatabase *db)
{
  ...
  vector feeds;
  VectorNew(&feeds,sizeof(feedPackage),FreeFeedVector,0);
...
static void *ProcessFeed(void *feedPack)
{
  feedPackage *fp = (feedPackage*)feedPack;
  sem_wait(&fp->db->totalNetworkConnections);
  url u;
  urlconnection urlconn;

  URLNewAbsolute(&u, fp->remoteDocumentName);
  ...
  switch (urlconn.responseCode) {
    ...
    case 200: PullAllNewsItems(&urlconn, fp->db);
              break;
...
static void ParseArticle(const char *articleTitle,const char *articleDescription,
			 const char *articleURL,rssDatabase *db, bool needToFreeConnection)
{
  url u;
  urlconnection urlconn;

  sem_wait(&db->totalNetworkConnections);
  ...
  switch (urlconn.responseCode) {
      case 0:   printf("Unable to connect to \"%s\". Domain name or IP address is nonexistent.\n", 
	        ...
	        break;
      case 200:
	        sem_wait(&db->articlesSeenLock);
	        if(HashSetLookup(&db->articlesSeen,&currArt) == NULL){ //if we haven't seen this article before
		  ParseArticleHelper(&currArt,&urlconn,&u,db);
		  break;
		}
	...
	{       // close network connection on redirect to prevent deadlock
      ...
      default:  printf("Unable to pull \"%s\" from \"%s\". [Response code: %d] Punting...\n", articleTitle, 
		       u.serverName, urlconn.responseCode);
		ArticleFree(&currArt);
	        break;
  }
...
void ParseArticleHelper(article *currArt, urlconnection *urlconn, url *u, rssDatabase *db)
{
  streamtokenizer st;
  printf("[%s] Indexing \"%s\"\n", u->serverName,currArt->title);
  HashSetEnter(&db->articlesSeen, currArt);
  sem_post(&db->articlesSeenLock);
  STNew(&st, urlconn->dataStream, kTextDelimiters, false);
  ...
  scanPack.db = db;
  scanPack.st = st;
  scanPack.currArt = *currArt;

  pthread_t thread;		  
  pthread_create(&thread,NULL,ScanArticle,(void*)&scanPack);
 
  sem_wait(&scanPack.scanArticleLock);
  STDispose(&st);
  sem_destroy(&scanPack.scanArticleLock);  
}
...
static void *ScanArticle(void *scanPackage)
{
  scanArticlePackage *scanPack = (scanArticlePackage*)scanPackage;

  char word[1024];

  while (STNextToken(&scanPack->st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      SkipIrrelevantContent(&scanPack->st); // in html-utls.h
    } else {
      RemoveEscapeCharacters(word);
      if (WordIsWellFormed(word)) {
	ProcessWellFormedWord(word,&scanPack->currArt,scanPack->db);
      }
    }
  }
...
{   
  ...
  sem_wait(&db->stopWordsLock);
  if(HashSetLookup(&db->stopWords, &word2) == NULL)  { //not a stopword
    sem_post(&db->stopWordsLock);
    w.thisWord = word2;	  
    VectorNew(&w.articles, sizeof(article),NULL, 100);
    sem_wait(&db->wordHashLock);
    currWord* elemAddr = (currWord*)HashSetLookup(&db->wordHash,&w);

    if(elemAddr == NULL){ // Hasn't been seen
      a->numOccurrences = 1;
      VectorAppend(&w.articles, a);
      HashSetEnter(&db->wordHash, &w);
    } else {
...
static void ProcessValidResponse(const char *word, rssDatabase *db)
{
  ...
  curr.thisWord = test;
  currWord* elemAddr = (currWord*)HashSetLookup(&db->wordHash,&curr);
